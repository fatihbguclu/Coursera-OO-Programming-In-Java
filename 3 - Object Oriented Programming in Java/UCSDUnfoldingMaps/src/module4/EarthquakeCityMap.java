	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			...
			for(Marker qm : quakeMarkers) {
				if(((EarthquakeMarker) qm).isOnLand() && qm.getProperty("country").equals(cm.getProperty("name")) ) {
					landCount +=1;
				}else if(!((EarthquakeMarker) qm).isOnLand()) {
					a++;
				}
			}
			if(landCount > 0) {
				System.out.println(cm.getProperty("name") + ": " + landCount);
			}
			landCount = 0;
